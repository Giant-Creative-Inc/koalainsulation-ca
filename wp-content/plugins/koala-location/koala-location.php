...
function koala_location_remove_slug($post_link, $post)
{
    if ($post->post_type === 'location' && $post->post_status === 'publish') {
        // Ensure the permalink doesn't include any slug
...
function koala_post_types_parse($query)
{
    ...
    if (count($uri_segments) == 2 && $query->is_main_query()) {
...
{
    register_post_type('service', array('labels' => array('name' => 'Services'), 'public' => true, 'rewrite' => array(
        'slug' => 'services',
    ), 'show_in_rest' => true));
}
...
{
    register_post_type('location-service', array('labels' => array('name' => 'Location Services'), 'public' => true, 'rewrite' => 'affiliate', 'show_in_rest' => true));
}
...
function koala_location_service_remove_slug($post_link, $post)
{
    if ($post->post_type == "location-service" && $post->post_status == "publish") {
        ...
        if ($related_location_id) {
			...
			//$post_link = home_url('/' . $related_location_slug . '/services/' . $post->post_name . '/');
        }
    }
