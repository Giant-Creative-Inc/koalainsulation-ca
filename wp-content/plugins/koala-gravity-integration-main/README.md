...
Immediately before the background job builds the n8n payload, the submitted mapped `zip` value is normalized and checked against the Location CPT's `location_zipcode` and comma-separated `additional_zipcodes` ACF fields. If another published location owns that exact normalized value, that location supplies `location_id`, `location_name`, `housecall_pro_api_key`, `location_serviceminder_api_key`, and `location_serviceminder_id` to n8n. The submitted entry values and `page_url` remain unchanged.

When there is no exact owner, zipcodeapi.com searches the configured 60-mile radius. Canadian requests use the API's high-limit `simple` response so dense markets are not restricted to the default nearest 250 postal codes; U.S. requests retain their existing endpoint and response format. A successful complete lookup with no owner in range marks the lead `unresolved`. If the API key is missing, the provider rate-limits the request, the transport fails, the provider reports an error, or the response is invalid, the page-derived location is preserved instead. The Gravity Forms entry's **ZIP Routing** value records the diagnostic type, for example `api_error_rate_limited_original_preserved` or `api_error_transport_original_preserved`.
