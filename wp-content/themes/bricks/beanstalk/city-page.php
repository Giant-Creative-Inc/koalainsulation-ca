...
function koala_beanstalk_replace_location_placeholders( string $content, ?array $context = null ): string {
	...
	foreach ( koala_beanstalk_get_location_placeholders() as $placeholder => $context_key ) {
		if ( in_array( $placeholder, array( '{{location_phone}}', '{{location_home_url}}' ), true ) ) {
			continue;
		}
	...
	$phone     = isset( $context['phone'] ) && is_scalar( $context['phone'] ) ? (string) $context['phone'] : '';
	$phone_url = isset( $context['phone_url'] ) && is_scalar( $context['phone_url'] ) ? (string) $context['phone_url'] : '';
	$phone_url = $phone_url ? $phone_url : ( $phone ? 'tel:' . preg_replace( '/[^0-9+]/', '', $phone ) : '' );

	$replacements['{{location_phone}}'] = $phone && $phone_url
		? '<a href="' . esc_url( $phone_url ) . '">' . esc_html( $phone ) . '</a>'
		: '';

	$location_name = isset( $context['location_name'] ) && is_scalar( $context['location_name'] ) ? (string) $context['location_name'] : '';
	$state_name    = isset( $context['state_name'] ) && is_scalar( $context['state_name'] ) ? (string) $context['state_name'] : '';
	$home_url      = isset( $context['home_url'] ) && is_scalar( $context['home_url'] ) ? (string) $context['home_url'] : '';
	$link_label    = implode( ', ', array_filter( array( $location_name, $state_name ) ) );

	$replacements['{{location_home_url}}'] = $home_url && $link_label
		? '<a href="' . esc_url( $home_url ) . '">' . esc_html( $link_label ) . '</a>'
		: '';
