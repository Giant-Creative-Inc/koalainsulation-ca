...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // const cachedUrl = sessionStorage.getItem("location_url");
  // const cachedName = sessionStorage.getItem("location_name");
  // const cachedServices = JSON.parse(sessionStorage.getItem("location_service") || "[]");

  // if (cachedUrl) {
  //   console.log("Applying cached url data...");

  //   // 1. Update Core Links
  //   const linkMap = {
  //     "nav-home": cachedUrl,
  //     "ft-home": cachedUrl,
  //     "why-koala-link": cachedUrl + "/why-koala",
  //     "why-koala-link-footer": cachedUrl + "/why-koala",
  //     "why-reinsulate-link": cachedUrl + "/why-reinsulate",
  //     "why-reinsulate-link-footer": cachedUrl + "/why-reinsulate",
  //     "homeowner-link": cachedUrl + "/homeowner-incentives",
  //     "homeowner-link-footer": cachedUrl + "/homeowner-incentives",
  //     "faq-nav-link": cachedUrl + "/faq",
  //     "faq-ft-link": cachedUrl + "/faq",
  //     "testimonials-nav-link": cachedUrl + "/testimonials",
  //     "testimonials-ft-link": cachedUrl + "/testimonials",
  //     "blog-link-nav": cachedUrl + "/blog",
  //     "blog-link-footer": cachedUrl + "/blog"
  //   };
  ...
  //   if (document.getElementById("loc-name") && cachedName) {
  //     document.getElementById("loc-name").textContent = cachedName;
  //   }

  //   const showIds = ["custom-service", "custom-service-footer", "location-nav-box", "nav-us"];
  //   showIds.forEach(id => {
  //     let el = document.getElementById(id);
  //     if (el) { el.style.display = "block"; el.style.visibility = "visible"; }
  //   });

  //   const hideIds = ["main-nav-services", "main-ft-services", "default-nav-box"];
  //   hideIds.forEach(id => {
  //     let el = document.getElementById(id);
  //     if (el) { el.style.display = "none"; el.style.visibility = "hidden"; }
  //   });

  //   const ul = document.getElementById("custom-service-ul");
  //   if (ul && cachedServices.length > 0 && ul.children.length === 0) {
  //     cachedServices.forEach(service => {
  //       let li = document.createElement("li");
  //       li.className = "menu-item";
  //       li.innerHTML = `<div class="brxe-xvkivb brxe-div"><a class="brxe-yttxdw brxe-text-link nav-nested-link" href="${service.link}">${service.title}</a></div>`;
  //       ul.appendChild(li);
  //     });
  //   }
  // }

  // reCAPTCHA is loaded by the shared interaction loader in functions.php.
  ...
  const navLink = sessionStorage.getItem("nav_top_link");
  ...
  if (locationNavTextUs) {
    ...
    if (navLink) {
      locationNavTextUs.href = navLink;
    }
  }
  ...
  if (usFooter) {
    ...
    usFooter.style.visibility = "hidden";
  }

  if (usSlider) {
    usSlider.style.display = "none";
    usSlider.style.visibility = "hidden";
  }

  if (usOurServices) {
    usOurServices.style.display = "none";
    usOurServices.style.visibility = "hidden";
  }

  if (usOurServicesSubnav) {
    usOurServicesSubnav.style.display = "none";
    usOurServicesSubnav.style.visibility = "hidden";
  }

  if (elementShow) {
    elementShow.style.display = "none";
    elementShow.style.visibility = "hidden";
  }

  if (elementDropDownShow) {
    elementDropDownShow.style.display = "none";
    elementDropDownShow.style.visibility = "hidden";
  }

  if (elementFooterShow) {
    elementFooterShow.style.display = "none";
    elementFooterShow.style.visibility = "hidden";
  }

  if (elementMainWidget) {
    elementMainWidget.style.display = "none";
    elementMainWidget.style.visibility = "hidden";
  }

  if (elementLocalWidget) {
    elementLocalWidget.style.display = "none";
    elementLocalWidget.style.visibility = "hidden";
  }

  if (elementMainStoriesWidget) {
    elementMainStoriesWidget.style.display = "none";
    elementMainStoriesWidget.style.visibility = "hidden";
  }

  if (elementLocalStoriesWidget) {
    elementLocalStoriesWidget.style.display = "none";
    elementLocalStoriesWidget.style.visibility = "hidden";
  }
  ...
  if (ctaAlt) {
    ctaAlt.style.display = "none";
    ctaAlt.style.visibility = "hidden";
  }

  if (ctaAlt2) {
    ctaAlt2.style.display = "none";
    ctaAlt2.style.visibility = "hidden";
  }

  if (ctaQuote) {
    ctaQuote.style.display = "none";
    ctaQuote.style.visibility = "hidden";
  }

  if (nationalNavQuote) {
    nationalNavQuote.style.display = "none";
    nationalNavQuote.style.visibility = "hidden";
  }

  if (nationalEstimateBtn) {
    nationalEstimateBtn.style.display = "none";
    nationalEstimateBtn.style.visibility = "hidden";
  }

  if (nationalEstimateFooterBtn) {
    nationalEstimateFooterBtn.style.display = "none";
    nationalEstimateFooterBtn.style.visibility = "hidden";
  }

  if (localEstimateBtn) {
    localEstimateBtn.style.display = "none";
    localEstimateBtn.style.visibility = "hidden";
  }

  if (localEstimateBtn1) {
    localEstimateBtn1.style.display = "none";
    localEstimateBtn1.style.visibility = "hidden";
  }

  if (areasServedNavLink) {
    areasServedNavLink.style.display = "none";
    areasServedNavLink.style.visibility = "hidden";
  }
  ...
  if (segments.length > 1) {
    const locationSlug = segments[1]; // Assume the first segment is the location slug

    if (locationSlug == 'why-koala' ||
        locationSlug == 'why-reinsulate' ||
        locationSlug == 'homeowner-incentives' ||
        locationSlug == 'locations' ||
        locationSlug == 'terms-and-condition' ||
        locationSlug == 'testimonials' ||
        locationSlug == 'blog' ||
        locationSlug == 'faq' ||
        locationSlug == 'resources-landing-pa'
    ){
      //return;
    }

    // Fetch location data from the REST API
    fetch(`/ca/wp-json/custom/v1/location-data/${locationSlug}?_t=${Date.now()}`)
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        ...
        if (data) {
          ...
          if (data.state === 'Ontario') {
            ...
            homeownerNavLink.style.display = "none";
            homeownerFooterLink.style.display = "none";
          }
        ...
        if (data.state) {
          sessionStorage.setItem("location_state", data.state);
        } else {
          sessionStorage.setItem("location_state", "");
        }
        ...
        if (data.phone1) {
          const phone1Wrapper = document.getElementById("phone1-wrapper");
          const telephoneLink = document.getElementById("tel-href");
          const locationPhone = document.getElementById("loc-phone-number");
          const footerPhone = document.getElementById("footer-number");

          if (phone1Wrapper) phone1Wrapper.href = "tel:" + data.phone1;
          if (telephoneLink) telephoneLink.href = "tel:" + data.phone1;
          if (locationPhone) locationPhone.textContent = data.phone1;
          if (footerPhone) {
            footerPhone.textContent = data.phone1;
            footerPhone.href = "tel:" + data.phone1;
          }
          ...
          if (phone1Wrapper) phone1Wrapper.href = "tel:(833)-335-6252";
          if (locationPhone) locationPhone.textContent = "(833)-335-6252";
        }

        if (data.phone2) {
          const phone2Wrapper = document.getElementById("phone2-wrapper");
          const secondaryPhone = document.getElementById("loc-phone-number2");

          if (phone2Wrapper) {
            phone2Wrapper.href = "tel:" + data.phone2;
            phone2Wrapper.style.display = "flex";
          }
          if (secondaryPhone) secondaryPhone.textContent = data.phone2;
          ...
          if (secondaryPhone) secondaryPhone.textContent = "";
        }
        ...
        if (data.nicejobId) {
          ...
          script.src =
            "https://cdn.nicejob.co/js/sdk.min.js?id=" + data.nicejobId;
        ...
        if (data.scriptInHead) {
          sessionStorage.setItem("location_before_head_tag", data.scriptInHead);

          // Singular location pages already queue this markup in PHP. Virtual
          // location routes receive it here and use the same interaction gate.
          if (!window.koalaHeadScriptOutput) {
            window.koalaHeadScriptOutput = true;
            window.koalaLocationHeadMarkup = data.scriptInHead;
            if (
        ...
        if (data.scriptInBody) {
          ...
          if (!window.koalaBodyScriptOutput) {
            window.koalaBodyScriptOutput = true;
            window.koalaLocationBodyMarkup = data.scriptInBody;
            if (
              window.koalaInteractionScriptsLoaded &&
              window.koalaInjectLocationMarkup
            ) {
        ...
        if (data.navLink) {
          document.getElementById("nav-top-text-us").href = data.navLink;
          sessionStorage.setItem("nav_top_link", data.navLink);
        } else {
      ...
      .catch((error) => {
        ...
        // /locations, /faq) - an expected case handled by the national fallback
        // below; only log genuinely unexpected failures.
...
jQuery(document).ready(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(".acc-head4").on("click", function () {
      console.log('acc-container4 ', $(this));
      const $accordionHead = $(this);
